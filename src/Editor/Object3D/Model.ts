...
export default class Model extends ObjectBase {
  data: BlueprintFurnitureJSON | undefined

  constructor(dataObject: BlueprintFurnitureJSON) {
    super()
  ...
  async create() {
    try {
      ...
      const modelJSON = modelsData.find(
        (model) => model.code === this.data?.ID.toString()
      )
      if (!modelJSON)
        throw new Error(`model ${this.data.ID} not exist in model JSON`)

      const model: THREE.Group = await ExpoTHREE.loadAsync(
        modelJSON.files.fbx.url
      )
      model.userData = this.data
      ...
      const { Position, Angle, Scale } = this.data.m_Tranform
      // set Position
